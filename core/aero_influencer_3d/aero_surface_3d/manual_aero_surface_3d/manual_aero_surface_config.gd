...
#@export_range(-100.0, 0.0, 0.001, "or_greater", "exp", "suffix:cL") var min_lift_coefficient : float = -1.2
###Lift coefficient used where the lift_aoa_curve has a value of 1.
#@export_range(0.0, 100.0, 0.001, "or_greater", "exp", "suffix:cL") var max_lift_coefficient : float = 1.2
##Curve that determines the ManualAeroSurface3D's lift coefficient, depending on the ManualAeroSurface3D's angle of attack.
@export var lift_aoa_curve : Curve = LiftCurve.new()
