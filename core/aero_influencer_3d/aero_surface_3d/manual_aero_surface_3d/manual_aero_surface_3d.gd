...
func _calculate_forces(substep_delta : float = 0.0) -> PackedVector3Array:
	...
	var aero_reference := get_dynamic_pressure() * area
	
	lift_force = aero_reference * manual_config.get_lift_coefficient(angle_of_attack)
	var drag_coefficient : float = manual_config.get_drag_coefficient(angle_of_attack) * manual_config.get_drag_at_sweep_angle(sweep_angle) * manual_config.get_drag_multiplier_at_mach(get_mach())
	var form_drag : float = aero_reference * drag_coefficient
	var induced_drag : float = (lift_force * lift_force) / (get_dynamic_pressure() * PI * wing_config.span * wing_config.span)
	#dynamic pressure causes divide by zero when airspeed is 0, which results in NAN.
	if is_equal_approx(get_air_speed(), 0.0):
		induced_drag = 0.0
...
#	the cosine of the sweep angle.
	...
	var drag_vector : Vector3 = get_drag_direction() * drag_force
	...
	set_current_force(force)
	set_current_torque(torque)
