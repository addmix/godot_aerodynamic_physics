...
func _calculate_forces(_world_air_velocity : Vector3, _air_density : float, _relative_position : Vector3, _altitude : float, substep_delta : float = 0.0) -> PackedVector3Array:
	super._calculate_forces(_world_air_velocity, _air_density, _relative_position, _altitude)
	...
	for influencer : AeroInfluencer3D in aero_influencers:
		...
		var force_and_torque : PackedVector3Array = influencer._calculate_forces(world_air_velocity + (-angular_velocity).cross(influencer_relative_position), air_density, relative_position, position.y, substep_delta)
